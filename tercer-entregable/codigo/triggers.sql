...
CREATE OR REPLACE TRIGGER TRANSMISION_MENSAJE
...
CREATE OR REPLACE TRIGGER PLAZAS_INSCRIPCION
BEFORE INSERT ON INSCRIPCIONES
FOR EACH ROW
DECLARE
    n_inscritos INTEGER;
...
BEGIN
    ...
    IF (SYSDATE NOT BETWEEN w_fechaCreacion AND w_fechaCreacion+15) THEN
        raise_application_error(-20602, 'Solo puede responderse al formulario durante los primeros 15 días desde su fecha de creación.');
    END IF;
END PLAZO_CUESTIONARIO;
/

CREATE OR REPLACE TRIGGER INTERES_INICIAL_ACT
